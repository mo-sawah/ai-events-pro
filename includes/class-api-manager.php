    ...
    public function get_events($location = '', $radius = 25, $limit = 20) {
        ...
        if (!empty($enabled_apis['eventbrite']) && !empty($eventbrite_settings['private_token'])) {
            ...
            try {
                ...
                if (is_wp_error($eventbrite_events)) {
                    $this->debug_log("Eventbrite error: " . $eventbrite_events->get_error_message());
                } else {
                ...
                $this->debug_log("Eventbrite exception: " . $e->getMessage());
            }
        } else {
        ...
        if (!empty($enabled_apis['ticketmaster']) && !empty($ticketmaster_settings['consumer_key'])) {
            ...
            try {
                ...
                if (is_wp_error($ticketmaster_events)) {
                    $this->debug_log("Ticketmaster error: " . $ticketmaster_events->get_error_message());
                } else {
                ...
                $this->debug_log("Ticketmaster exception: " . $e->getMessage());
            }
        } else {
